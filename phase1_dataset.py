...
def process_shape(sid, run_tests=False):
    ...
    if not os.path.exists(fbx) or os.path.getsize(fbx) == 0:
        return "skip"   # empty/missing — not a real failure

    # Load FBX with fbxloader (pure-Python, no Blender required)
    try:
        loader = FBXLoader(fbx)
        tm = loader.export_trimesh()
    except Exception:
        return None
    ...
    # Convert trimesh → Open3D for surface sampling
    ...
    # Sample 1024 surface points with outward normals
    pts, nrm, n_flip = sample_surface(mesh, args.num_points)

    # Rig extraction (requires Blender + blender_extract_rig.py)
    rig = extract_rig(fbx)
    if rig is not None:
        rig = align_joints(rig, verts)

    bfs_seq  = build_bfs(rig['joint_names'], rig['parents'].tolist()) if rig else None
    ...
    return dict(pts=pts, nrm=nrm, bfs_seq=bfs_seq, rig=rig, skin=skin,
                n_joints=n_joints, n_flip=n_flip)
...
def main():
    ...
    n_valid  = sum(1 for sid in shape_ids
                   if os.path.exists(os.path.join(args.fbx_dir, f"{sid}.fbx"))
                   and os.path.getsize(os.path.join(args.fbx_dir, f"{sid}.fbx")) > 0)
    n_empty  = len(shape_ids) - n_valid
    ok(f"Shapes queued: {len(shape_ids)}  ({n_valid} with data, {n_empty} empty/missing)")
    if n_empty > 0:
        warn(f"{n_empty} FBX files are 0 bytes — dataset may be incomplete")
        warn("Re-download missing files from the RigNet dataset source")

    # ── Find first valid shape for test checkpoints ──
    first = next((sid for sid in shape_ids
                  if os.path.exists(os.path.join(args.fbx_dir, f"{sid}.fbx"))
                  and os.path.getsize(os.path.join(args.fbx_dir, f"{sid}.fbx")) > 0), None)
    if first is None:
        err("No valid FBX files found — check dataset download"); sys.exit(1)
    ...
    if args.resume and already_done(first):
        ...
        if r is None or r == "skip":
            err(f"First valid shape {first} failed"); sys.exit(1)
    ...
    if n_empty_skip: info(f"Empty:   {n_empty_skip} (0-byte FBX — dataset incomplete)")
